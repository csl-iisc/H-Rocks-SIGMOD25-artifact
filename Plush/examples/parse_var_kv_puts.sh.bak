#!/usr/bin/env bash
# Usage: ./plush_parse_var_kv_puts_min.sh [INPUT_DIR] [OUTPUT_CSV]
# Ex:    ./plush_parse_var_kv_puts_min.sh output_var_kv_puts plush_var_kv_puts.csv
# Notes: expects files named output_<count>_<k>_<v>
# Env filters (optional): SIZES="5000000 10000000" KEYS="8 16" VALS="8 32 128"
set -euo pipefail

IN_DIR="${1:-output_var_kv_puts}"
OUT_CSV="${2:-plush_var_kv_puts.csv}"
SIZES_LIST="${SIZES:-}"
KEYS_LIST="${KEYS:-}"
VALS_LIST="${VALS:-}"
...
for f in "$IN_DIR"/output_*_*_*; do
  ...
  # grab last field on the first 'put_time' line (assumed milliseconds)
  ms="$(grep -m1 -E 'put_time' "$f" 2>/dev/null | awk '{print $NF}')"; : "${ms:=0}"
