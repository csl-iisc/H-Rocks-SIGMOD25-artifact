...
int main(int argc, char **argv) 
{
    ...
    while ((option_char = getopt (argc, argv, ":p:n:k:v:t:")) != -1){
        switch (option_char)
        {
            ...
            case 'n': num_removes = atoi (optarg); break;
...
#pragma omp parallel for num_threads(nthreads)
    ...
    auto remove_time = (TIME_NOW - start).count(); 
    std::cout << "remove_time: " << remove_time/1000000.0 << "\n"; 
    insert_time = remove_time/1000000.0;
    std::cout << "remove throughput: " << float(num_removes)/remove_time/1000.0 << " Mops/sec\n"; 
