...
# Usage:
#   ./viper_parse_ycsbC.sh [INPUT_DIR] [OUTPUT_CSV]
# Example:
#   ./viper_parse_ycsbC.sh output_ycsbC viper_ycsbC_throughput.csv
...
for f in "$IN_DIR"/output_*; do
  ...
  gt_line="$(grep -m1 -E 'get_time|final_get_time' "$f" 2>/dev/null || true)"
