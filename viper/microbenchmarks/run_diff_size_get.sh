...
for keys in $NKEYS; do
    for key_size in $KEY_SIZES; do
        for val_size in $VAL_SIZES; do
            ...
            ./build/prefill_get -p $PREFILL_SIZE -n $keys -k $key_size -v $val_size -t 128 > output_diff_sizes_get/output_10M_${key_size}_${val_size}
        done
    done
done
