...
void threaded_func2(btree* bt, char* value, uint64_t start, uint64_t end, int key2) {
    for(uint64_t i = start; i < end; ++i) {
        int i_value = std::stoi(value);
        i_value++;
        key2++; 
        ...
        char* obtained_value = bt->search(key2);
        bt->remove(key2);                   
...
int main(int argc, char **argv) 
{
    ...
    strcpy(value1, "1000"); 
    char* value2 = (char*) malloc(100); 
    strcpy(value2, "1000"); 
    std::thread t1(threaded_func1, bt, value1, 0, num_puts, key);
    std::thread t2(threaded_func2, bt, value2, 0, num_puts, key2);
