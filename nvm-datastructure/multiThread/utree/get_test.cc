...
#pragma omp parallel for num_threads(nthreads)
    for(uint64_t i = 0; i < num_gets; ++i) {
        int idx = rand()%num_puts; 
        obtained_value = bt->search(keys[idx]); 
    }
    auto get_time = (TIME_NOW - start).count(); 
    std::cout << "get_time: " << get_time/1000000.0 << "\n"; 
    insert_time = get_time/1000000.0;
    std::cout << "throughput: " << float(num_gets)/get_time/1000.0 << " Mops/sec\n"; 
