#!/usr/bin/env bash
# parse_ycsbB_min.sh
# Usage: ./parse_ycsbB_min.sh [INPUT_DIR] [OUTPUT_CSV]
set -euo pipefail
IN_DIR="${1:-.}"
...
for f in "$IN_DIR"/output_*_8_8; do
  ...
  ws="$(get_num 'setup_time|update_setup_time' "$f")"; wso="$(get_num 'sort_time|update_sort_time' "$f")"
  ww="$(get_num 'write_batch_time|update_write_batch_time' "$f")"; wm="$(get_num 'memtable_time|update_memtable_time' "$f")"
