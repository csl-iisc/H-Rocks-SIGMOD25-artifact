...
__device__ 
unsigned long long hash_fnv1a_device(const char* key, int length) {
...
__host__ __device__
bool string_compare2(const char* a, const char* b, size_t length) {
...
__device__ 
bool cacheRead(BCache* cache, const char* key, char** value, int keyLength) {
    unsigned long long hashedKey = hash_fnv1a_device(key, keyLength);
    ...
    for (int i = 0; i < NUM_WAYS; i++) {
        if (set->lines[i].tag == tag && !string_compare2(set->lines[i].key, key, keyLength) == 0) {
...
__host__ __device__ uint64_t findValidIndex(const Memtable *memtable, const char* key, int keyLength, uint64_t query_operationID, uint64_t index, uint64_t* opID) {
    ...
    if (opID[index] > query_operationID) {
        while (index > 0 && string_compare2(memtable->d_sortedKeys + (index - 1) * keyLength, key, keyLength) && opID[index - 1] > query_operationID) {
            index--;
        }

    } else {
        while (index < memtable->size - 1 && string_compare2(memtable->d_sortedKeys + (index + 1) * keyLength, key, keyLength) && opID[index + 1] < query_operationID) {
            index++;
        }
...
__global__ void debugging(Memtable *active, int size) 
{
    for(int i = 0; i < size; i++) 
        printf("%s %d\n", active->d_sortedKeys + (i * 8), active->d_sortedOperationIDs[i]); 
}

__global__ void search(const Memtable *active, const Memtable *immutable, const char* queryData, const uint64_t numQueries, char** results, int keyLength, int* idx_result, uint64_t* query_operationID, BCache* cache, unsigned int* notFoundIdx, char* notFound, uint64_t* active_opID, uint64_t* immutable_opID, int active_batchID, int immutable_batchID) 
{
    ...
    for(uint64_t i = tid; i < numQueries; i += NTHREADS_PER_BLK * NBLKS) {
        ...
        if (binarySearch(active->d_sortedKeys, 0, active->size - 1, key, index, keyLength)) {
            if ((index > 0 && string_compare2(active->d_sortedKeys + (index - 1) * keyLength, key, keyLength)) || (index < active->size - 1 && string_compare2(active->d_sortedKeys + (index + 1) * keyLength, key, keyLength))) {
#ifdef __PRINT_DEBUG__
            ...
            //printf("%d, %d\n", tid, index); 
            idx_result[i] = index;
            results[i] = active->d_sortedValuePointers[index];  
            found = true; 
        } else if (binarySearch(immutable->d_sortedKeys, 0, immutable->size - 1, key, index, keyLength)) {
            if ((index > 0 && string_compare2(immutable->d_sortedKeys + (index - 1) * keyLength, key, keyLength)) || (index < immutable->size - 1 && string_compare2(immutable->d_sortedKeys + (index + 1) * keyLength, key, keyLength))) {
                index = findValidIndex(immutable, key, keyLength, query_operationID[i], index, immutable_opID);
            }
            idx_result[i] = index;
            results[i] = immutable->d_sortedValuePointers[index]; 
            found = true; 
        } 
        }
        // TODO: add code for when the batchID is same 
            ...
            found = cacheRead(cache, key, &results[i], keyLength);
        }
        if (!found) {
...
void searchGetsOnGPU(char* getKeys, Memtable* activeMemtable, Memtable* immutableMemtable, int keyLength, uint64_t numReads, char** getValuePointers, uint64_t* getOperationIds, BCache* cache)
{
    ...
    *notFoundIdx = 0;
    ...
    char **d_getValuePointers; 
    cudaMalloc(&d_getValuePointers, numReads * sizeof(char*));
    ...
    start_time = TIME_NOW; 
    search<<<NBLKS, NTHREADS_PER_BLK>>>(activeMemtable, immutableMemtable, d_queryData, numReads, d_getValuePointers, keyLength, resultIdx, d_getOperationIds, cache, notFoundIdx, notFound, activeMemtable->d_sortedOperationIDs, immutableMemtable->d_sortedOperationIDs, activeMemtable->batchID, immutableMemtable->batchID);
    cudaDeviceSynchronize(); 
    auto getKernelTime = (TIME_NOW - start_time).count();
    err = cudaPeekAtLastError();
    printf("Error %d cudaPeekerror\n", err);
    getValuePointers = (char**)malloc(numReads * sizeof(char*)); 
    cudaHostRegister(getValuePointers, numReads * sizeof(char*), 0); 
    start_time = TIME_NOW; 
    cudaMemcpy(getValuePointers, d_getValuePointers, numReads * sizeof(char*), cudaMemcpyDeviceToHost);
