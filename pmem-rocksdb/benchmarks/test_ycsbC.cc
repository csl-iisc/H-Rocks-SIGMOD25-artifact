...
#pragma omp critical
  ...
  // ---------------------- Sampler for reads --------------------------
  ScrambledZipf zipf(prefill, (use_uniform ? 0.99 : zipf_theta)); // theta ignored if uniform
  std::uniform_int_distribution<uint64_t> U(0, prefill - 1);

  std::atomic<uint64_t> read_ok{0}, read_miss{0};
...
#pragma omp critical
          std::cerr << "Warning: value size mismatch at op=" << i
